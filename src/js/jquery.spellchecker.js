 ...
 */
 ...
 (function(window, $) {
  ...
  Parser.prototype.clean = function(text) {
    ...
    var puncExpr = [
      unicodePunctuation + '+\\s+' + unicodePunctuation + '+', // punctuation(s) with leading and trailing whitespace(s)
      unicodePunctuation + '+\\s+',                            // puncutation(s) with trailing whitespace(s)
      '\\s+' + unicodePunctuation + '+',                       // punctuation(s) with leading whitespace(s)
    ].join('|');

    text = text.replace(XRegExp(puncExpr, 'g'), ' '); // strip any punctuation
  ...
  HtmlParser.prototype.replaceWord = function(oldWord, replacement) {
    ...
    var replaceFill;
    var c;
    var regExp = XRegExp('(^|[^' + unicodeLetter + '])(' + oldWord + ')(?=[^' + unicodeLetter + ']|$)', 'g');
  ...
  HtmlParser.prototype.highlightWords = function(incorrectWords) {
    ...
    var regExp = XRegExp('(^|[^' + unicodeLetter + '])(' + incorrectWords.join('|') + ')(?=[^' + unicodeLetter + ']|$)', 'g');
    var c;
    var replaceElement;
...
window.findAndReplaceDOMText = (function() {
  ...
  function findAndReplaceDOMText(regex, node, replacementNode, captureGroup) {
    ...
    var m, indexes, matches = [], text = _getText(node);
    ...
    if (regex.global) {
      while (m = regex.exec(text)) {
        indexes = _getMatchIndexes(m, text, captureGroup);
   ...
   */
  function _getMatchIndexes(m, text, captureGroup) {
